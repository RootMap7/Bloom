...
class ReceiveCareModal extends StatefulWidget {
  final String? initialSelectedType;

  const ReceiveCareModal({
    super.key,
    this.initialSelectedType,
  });

  @override
  State<ReceiveCareModal> createState() => _ReceiveCareModalState();
}

class _ReceiveCareModalState extends State<ReceiveCareModal> {
  List<ReceiveCareOption> _options = [];
  ...
  Future<void> _loadOptions() async {
    try {
      final options = await ProfileDetailsService.fetchAllReceiveCareOptions();
      if (mounted) {
        setState(() {
          _options = options;
          _isLoading = false;
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to load care options: $e')),
        );
      }
    }
  }
  ...
  Widget _buildOptionCard(ReceiveCareOption option, bool isSelected) {
